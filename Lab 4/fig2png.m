...
% Step Response
file_name = "Report/Figures/.fig/step_response.fig";
step_fig = openfig(file_name);
step_fig.WindowState = 'maximized';
title_name = "Experiment 2: Step Response";
subtitle_name = "$k_c$ = 1.8154, $z$ = 8, $p$ = 20.7149";
...
file_name = "Report/Figures/exp2_step_response.png";
saveas(step_fig, file_name);
close(step_fig);

% Unit Ramp Response
file_name = "Report/Figures/.fig/unit_ramp_response.fig";
unit_ramp_fig = openfig(file_name);
unit_ramp_fig.WindowState = 'maximized';
title_name = "Experiment 2: Unit Ramp Response";
subtitle_name = "$k_c$ = 1.8154, $z$ = 8, $p$ = 20.7149";
servo_title = title(title_name);
servo_subtitle = subtitle(subtitle_name,'interpreter','latex');
xlabel('Time (s)'); ylabel('Servo Angle (degrees)');
...
legend(servo_ax,{'Input Signal', 'Output Signal'}, 'TextColor', '#AFAFAF');
set(gca, 'Units','normalized', 'Position',[0.07 0.1 0.9 0.8]);
unit_ramp_fig.InvertHardcopy = 'off';
file_name = "Report/Figures/exp2_unit_ramp_response.png";
saveas(unit_ramp_fig, file_name);
close(unit_ramp_fig);
